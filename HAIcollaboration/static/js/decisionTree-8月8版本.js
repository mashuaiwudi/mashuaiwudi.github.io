        ...
        success: function (data) { // data是接收的后台返回数据
            ...
            for (var i = 0; i < 15; i++) {
                ...
                if (rule_list[i] != 'None') {
                    ...
                    //console.log(tmp_data[0],valuechoices,tmp_data[2]);
                    if (tmp_data[0] == 'Label' || tmp_data[0] == 'Age' || tmp_data[0] == 'Work.hours.per.week' || tmp_data[0] == 'Education.num'){ 
                        for (var k = 0; k < valuechoices.length; k++) { // 单值渲染
                            /*
                            if (tmp_data[2] == valuechoices[k]) {
                                var tmp_index = k;
                                tmp_value[tmp_index + 1].selected = true;
                                console.log('loop',tmp_data[0]);  
                                break;
                            }*/
...
function chgRelation(idx) {
    ...
    if (feature != 0) { // 不结束，需要显示节点、画出对应的连线
        ...
        if (idx == '2-1') {
            ...
            for (var i = 0; i < 6; i++) {
                $("#feature" + list[i])[0].selected = true;
                $("#relation" + list[i]).find("option").remove();
                $("#value" + list[i]).find("option").remove();
                document.getElementById('node' + list[i]).style.display = "none";
            }
